...
const PORT = Number(process.env.PFFT_GPU_PORT || 8790);
...
const submitRpcUrl = process.env.PFFT_SUBMIT_RPC_URL || process.env.SUBMIT_RPC_URL || process.env.PRIVATE_RPC_URL || rpcUrl;
...
const noSubmit = args.has("--no-submit") || boolEnv("PFFT_GPU_NO_SUBMIT", false) || !account;
const minerAddress = account?.address || process.env.PFFT_MINER_ADDRESS?.trim() || process.env.MINER_ADDRESS?.trim();

if (!minerAddress) {
  throw new Error("PFFT GPU miner needs PFFT_PRIVATE_KEY or PFFT_MINER_ADDRESS");
}

const publicClient = createPublicClient({ chain: mainnet, transport: viemHttp(rpcUrl) });
const walletClient = account && !noSubmit
  ? createWalletClient({ account, chain: mainnet, transport: viemHttp(submitRpcUrl) })
  : null;
...
async function readState() {
  const [info, powTarget, powStage, powHexZeros, baseMintAmount, actualMintAmount, mintedByAddress, challenge] = await Promise.all([
    publicClient.readContract({ address: CONTRACT, abi: ABI, functionName: "getInfo" }),
    publicClient.readContract({ address: CONTRACT, abi: ABI, functionName: "POW_TARGET" }),
    publicClient.readContract({ address: CONTRACT, abi: ABI, functionName: "currentPowStage" }),
    publicClient.readContract({ address: CONTRACT, abi: ABI, functionName: "currentPowHexZeros" }),
    publicClient.readContract({ address: CONTRACT, abi: ABI, functionName: "BASE_MINT_AMOUNT" }),
    publicClient.readContract({ address: CONTRACT, abi: ABI, functionName: "BASE_MINT_AMOUNT" }).then((base) =>
      publicClient.readContract({ address: CONTRACT, abi: ABI, functionName: "calculateActualMint", args: [base] }),
    ),
    publicClient.readContract({ address: CONTRACT, abi: ABI, functionName: "minted", args: [minerAddress] }),
    publicClient.readContract({ address: CONTRACT, abi: ABI, functionName: "currentPowChallenge", args: [minerAddress] }),
  ]);
...
async function target() {
  ...
  return {
    ...
    difficulty: uint256Hex(state.powTarget),
    powStage: Number(state.powStage),
    powHexZeros: Number(state.powHexZeros),
    totalMinted: `${formatUnits(totalMinted, 18)} PFFT`,
    remainingSupply: `${formatUnits(remainingSupply, 18)} PFFT`,
    decay: `${(Number(decayBps) / 100).toFixed(2)}%`,
    nextMintAmount: `${formatUnits(nextMintAmount, 18)} PFFT`,
    actualMintAmount: `${formatUnits(state.actualMintAmount, 18)} PFFT`,
    mintedByAddress: `${formatUnits(state.mintedByAddress, 18)} PFFT`,
    baseMintAmount: `${formatUnits(state.baseMintAmount, 18)} PFFT`,
    submitRpcUrl,
    noSubmit,
  };
}
...
async function submitNonce(payload) {
  ...
  const claimedDifficulty = String(payload.difficulty || "");

  if (!/^0x[0-9a-fA-F]{64}$/.test(nonce)) throw new Error("invalid nonce");
  if (!/^0x[0-9a-fA-F]{64}$/.test(claimedChallenge)) throw new Error("invalid challenge");
  if (!/^0x[0-9a-fA-F]{64}$/.test(claimedDifficulty)) throw new Error("invalid difficulty");

  const fresh = await target();
  if (
    fresh.challenge.toLowerCase() !== claimedChallenge.toLowerCase()
    || fresh.difficulty.toLowerCase() !== claimedDifficulty.toLowerCase()
  ) {
    return { accepted: false, stale: true, message: "challenge or target changed before submit" };
  }
  ...
  if (noSubmit || !walletClient || !account) {
    return { accepted: true, submitted: false, message: "no-submit mode", hash: proof.hash };
  }
  ...
  const gas = gasEstimate < 180000n ? 180000n : gasEstimate > 350000n ? 350000n : (gasEstimate * 3n) / 2n;
  const priorityFee = optionalGwei("PFFT_PRIORITY_FEE_GWEI", "PRIORITY_FEE_GWEI");
  const maxFee = optionalGwei("PFFT_MAX_FEE_GWEI", "MAX_FEE_GWEI");
...
<head>
  ...
  <title>PFFT WebGPU Miner</title>
  <style>
    :root { color-scheme: dark; --bg:#091015; --fg:#eef5f7; --muted:#90a0a8; --line:rgba(94,211,255,.18); --accent:#57d6ff; --bad:#ff6b6b; --amber:#ffd56d; }
    * { box-sizing:border-box; }
    body { margin:0; background:radial-gradient(circle at top right, rgba(87,214,255,.08), transparent 30%), var(--bg); color:var(--fg); font:14px/1.45 ui-monospace, SFMono-Regular, Menlo, Consolas, monospace; }
    ...
    main { width:min(1160px,100%); margin:0 auto; padding:24px; display:grid; gap:18px; }
    .panel { border:1px solid var(--line); background:rgba(8,18,24,.9); padding:20px; }
    ...
    pre { margin:0; white-space:pre-wrap; max-height:300px; overflow:auto; color:#c8d9df; }
    a { color:var(--accent); }
    .ok { color:var(--accent); } .bad { color:var(--bad); } .warn { color:var(--amber); }
    @media (max-width:900px) { header,.hero { display:block; height:auto; } header { padding:16px; } .grid { grid-template-columns:repeat(2,minmax(0,1fr)); } }
  </style>
</head>
<body>
  <header>
  ...
  <main>
    ...
    <section class="panel grid">
      ...
      <div class="kv"><span>stage</span><strong id="powStage">--</strong></div>
      ...
      <div class="kv"><span>next mint</span><strong id="mintAmount">--</strong></div>
      <div class="kv"><span>wallet minted</span><strong id="mintedByAddress">--</strong></div>
      <div class="kv"><span>total minted</span><strong id="totalMinted">--</strong></div>
      <div class="kv"><span>remaining</span><strong id="remainingSupply">--</strong></div>
      <div class="kv"><span>decay</span><strong id="decay">--</strong></div>
...
class Miner {
  ...
  async init(){
    if(!navigator.gpu) throw new Error("WebGPU not available. Use current Chrome/Edge and open http://127.0.0.1:8790");
  ...
  async run(){
    ...
    $("wallet").textContent=target.miner;
    $("mode").textContent=target.noSubmit ? "no-submit" : "auto-submit";
    $("powStage").textContent="stage "+(target.powStage + 1)+"/5 · z"+target.powHexZeros;
    $("difficulty").textContent=target.difficulty.slice(0,10)+"..."+target.difficulty.slice(-6);
    $("challenge").textContent=target.challenge.slice(0,10)+"..."+target.challenge.slice(-6);
    $("mintAmount").textContent=target.actualMintAmount;
    $("mintedByAddress").textContent=target.mintedByAddress;
    $("totalMinted").textContent=target.totalMinted;
    $("remainingSupply").textContent=target.remainingSupply;
    $("decay").textContent=target.decay;
    ...
    while(this.running){
      ...
      if(r.found){
        ...
        if(submit.accepted && submit.submitted){ $("tx").innerHTML='<a class="ok" href="https://etherscan.io/tx/'+submit.tx+'" target="_blank">'+submit.tx.slice(0,10)+"..."+submit.tx.slice(-6)+"</a>"; log("submitted "+submit.tx+" block "+submit.blockNumber, "ok"); }
        else if(submit.accepted){ $("tx").textContent="not submitted"; log(submit.message || "accepted without submit", "warn"); }
...
let miner=null;
$("start").onclick=async()=>{ try{ $("status").textContent="starting"; if(!miner){ miner=new Miner(); await miner.init(); } miner.run().catch(e=>{ $("status").textContent="error"; log(e.message || e, "bad"); }); } catch(e){ $("status").textContent="error"; log(e.message || e, "bad"); } };
$("stop").onclick=()=>miner?.stop();
fetch("/api/target").then(r=>r.json()).then(t=>{ $("wallet").textContent=t.miner || "no wallet"; $("mode").textContent=t.noSubmit ? "no-submit" : "auto-submit"; $("powStage").textContent="stage "+(t.powStage + 1)+"/5 · z"+t.powHexZeros; $("difficulty").textContent=t.difficulty ? t.difficulty.slice(0,10)+"..."+t.difficulty.slice(-6) : "--"; $("challenge").textContent=t.challenge ? t.challenge.slice(0,10)+"..."+t.challenge.slice(-6) : "--"; $("mintAmount").textContent=t.actualMintAmount || "--"; $("mintedByAddress").textContent=t.mintedByAddress || "--"; $("totalMinted").textContent=t.totalMinted || "--"; $("remainingSupply").textContent=t.remainingSupply || "--"; $("decay").textContent=t.decay || "--"; if (autostart) { setTimeout(() => $("start").click(), 400); } }).catch(e=>log(e.message,"bad"));
</script>
</body>
</html>`;
...
server.listen(PORT, HOST, () => {
  console.log(`PFFT WebGPU miner: http://${HOST}:${PORT}`);
  console.log(`miner: ${minerAddress}`);
  console.log(`read rpc: ${rpcUrl}`);
  console.log(`submit rpc: ${submitRpcUrl}${noSubmit ? " (no-submit)" : ""}`);
});
